...
void BoxBoard::RegisterChain(LOC FreeBoxLoc, LOC FirstLoc)//从一个场内的自由格出发，给所有的派生链做一个定义。
{
...
void BoxBoard::SearchingFromBox(LOC BoxLoc)//从一个格出发，注册他的所有派生链,ChianPlus应仅在没有短链时启用。
{
	...
	for (int n = 0; n < 4; n++)
	{
		...
		if (board[lx][ly] == EDGE&&GetBoxType(bx, by) == CHAINBOX)//邻边为空，目标格子为链格
		{
			if (Boxes[bx][by].BelongingChainNum == EMPTY)//必须为未归属的格子，避免环重复从不同方向出发。
			{
				LOC NewLoc;
				NewLoc.Set(bx, by);
				RegisterChain(BoxLoc, NewLoc);//从一个场内的自由格出发，给所有的派生链做一个定义。
			}
		}
	}
	...
	for (int i = 1; i <= BOXLEN; i++)
	{
		for (int j = 1; j <= BOXLEN; j++)
		{
			if (Boxes[i][j].Type >= 3)//+
			{
				LOC k;
				k.Set(i, j);
				SearchingFromBox(k);//从一个格出发，注册他的所有派生链,ChianPlus应仅在没有短链时启用。
			}
		}
	}
...
//类型判断
void BoxBoard::RegisterDeadChain(LOC FreeBoxLoc, LOC FirstLoc)//从一个场内的死格出发，给其派生链做一个定义。
{
	//动态注册链。从一个格子出发 ，向一个格子开始进行链的注册。一般仅从自由格出发
...
void BoxBoard::SearchingDeadChain(LOC BoxLoc)//搜索死链
{
	int Dir[4][2] = { { 1, 0 }, { 0, 1 }, { -1, 0 }, { 0, -1 } };
	for (int n = 0; n < 4; n++)
	{
		...
		if (board[lx][ly] == EDGE&&GetBoxType(bx, by) == CHAINBOX)//邻边为空，目标格子为链格
		{
			if (Boxes[bx][by].BelongingChainNum == EMPTY)//必须为未归属的格子，避免环重复从不同方向出发。
			{
				LOC NewLoc;
				NewLoc.Set(bx, by);
				RegisterDeadChain(BoxLoc, NewLoc);//从一个场内的死格出发，给其派生链做一个定义
			}
		}
	}
}
void BoxBoard::DefineDeadChain()//检查所有的死格
{
	DefineBoxesType();//首先定义所有格子的类型
	ResetChainsInfo();//重置链的数据
	for (int i = 1; i <= BOXLEN; i++)
	{
		for (int j = 1; j <= BOXLEN; j++)
		{
			if (Boxes[i][j].Type == DEADBOX)
			{
				LOC k;
				k.Set(i, j);
				SearchingDeadChain(k);//从一个格出发，注册他的所有派生环。
			}
		}
	}
}
bool BoxBoard::GetDeadChainExist()//检查是否存在死链
{
	DefineDeadChain();//检查所有的死格
	for (int i = 0; i < BOXNUM; i++)
	{
...
bool BoxBoard::GetDeadCircleExist()//检查是否存在死环
{
	DefineDeadChain();//检查所有的死格
	for (int i = 0; i < BOXNUM; i++)
	{
