...
float UCTNode::RefreshAvgValue()//更新平均收益
{
	float Value = 0;
	//首先计算平均收益
...
float UCTNode::RefreshAvgValueWithSolver()
{
	...
	//首先计算平均收益
	bool Fail = true;
	for (int i = 0; i < ExistChild; i++)//判断当前模拟的胜利者
	{
...
float UCTNode::GetUCBValue(int Total)//得到UCB评估次数
{
...
float UCTProcess(UCTNode &B, int &Total, int MC_Times, int Filter_Range)
{
	...
	if (B.ExistChild < B.TotalChild)//如果还有未尝试过的节点，增加节点
	{
	...
	else//说明没有未尝试过的节点，开始评估
	{
		...
		UCTProcess(*B.ChildNodes[BestNodeNum], Total, MC_Times, Filter_Range);//循环评估直到到达时间或模拟游戏结束
		B.RefreshAvgValue();//更新平均收益
		return B.AvgValue;
	}
	return 0;
}

void UCTMove(Board &CB, int Player, bool Msg)//前期算法
{
	...
	{
		CB.GetAllCTypeBoxes(Player,true);//吃掉所有的C型格
...
void UCTMoveWithSacrifice(Board &CB,int Player,bool Msg,int branch)
{
	...
	if (DeadCircle||DeadChain)//有环的情况优先处理
	{
		...
		if (DeadChain)//然后处理死链
		{
			...
			//然后开始考虑DoubleCross
			DCMove = CB.GetDoubleCrossLoc(Player);	//查找一个doublecross的制作方法，返回值为该边的坐标
		}
		else
		{
			//首先吃到贪婪的临界点
		...
		//然后开始制造UCTProcess(开始uct模拟)
		UCTNode GreedyChild =    UCTNode(Player, CB.board, CB.Step, true, UCT_FILTER_RANGE);//根据当前局面创建贪婪算法UCT的根节点
		UCTNode SacrificeChild = UCTNode(-Player, CB.board, CB.Step, false, UCT_FILTER_RANGE);//根据当前局面创建牺牲算法UCT的根节点
		SacrificeChild.Move(DCMove.x, DCMove.y, Player);
		for (;;)//直到无法占据CTypeBox了就结束
		{
			if (!SacrificeChild.GetCTypeBoxLimit(Player, false))//限制版吃C型格
				break;
		}
...
void LatterSituationMove(Board &CB, int Player, bool Msg)
{
	...
	if (CB.GetLongCTypeBoxExist())
	{
		...
		BoxBoard Dead(CB);
		int SacrificeBoxNum;
		if (Dead.GetDeadChainExist())//检查是否存在死链
			SacrificeBoxNum = 2;
		if (Dead.GetDeadCircleExist())
			SacrificeBoxNum = 4;
...
void GameTurnMove(Board &CB, int Player, bool Msg)
{
	...
	bool LatterSituation = (Test.GetFilterMoveNum() == 0);//得到所有的过滤可行边的数量
...
void RaveSolverDouble(Board &CB, int Player, bool Msg)
{
	...
	bool LatterSituation = (Test.GetFilterMoveNum() == 0);
	//bool LongCTypeBoolExist = CB.GetLongCTypeBoxExist();
	if (!LatterSituation)//所有的过滤可行边的数量为零
	{
		//cout << "使用UCT" << endl;
