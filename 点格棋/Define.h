...
public:
	...
	bool GetLongCTypeBoxExist();					//判断局面中是否存在长C型链(错误)
...
bool IfOddNumber(int num);//判断一个数字是否是奇数
bool IfEvenNumber(int num);//判断一个数字是否是偶数
void MoveMsg(int x, int y, int Player);//占领Msg