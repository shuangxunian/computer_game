...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	{
		cprintf("默认模式开启", 8);
		printf("1:红方AI，2:蓝方AI\n");
		scanf("%d", &t);
	}
	else if (model == 1)//算法对弈模式
		Model_AI_Game(CB);
	else if (model == 2)//随机测试模式
		Model_Rnd_Test(CB);
	else if (model == 3)//测试胜利者
