...
int Board::GetBoxLiberties(int x,int y)//得到某个格子的自由度
{
	int num = board[x][y];
	if(num!=BOX&&num!=RED_BOX&&num!=BLUE_BOX)
	{
	...
	for (int j = 0; j < LEN; j++)
	{
		for (int i = 0; i < LEN; i++)
		{
			if (board[i][j] != 0 && ((IfOddNumber(j) && IfEvenNumber(i)) || (IfEvenNumber(j) && IfOddNumber(i))))
			{
				system("cls");
				PrintBoard();
				cout << "请输入[" << i << " , " << j << "]这条边的属主【0，1，2】输入99结束" << endl;
				int Num;
				cin >> Num;
				if (Num == 1)
					board[i][j] = 1;
	...
	for (int j = 0; j < LEN; j++)
	{
		for (int i = 0; i < LEN; i++)
		{
			if (board[i][j] != 0 &&IfOddNumber(j) && IfOddNumber(i))
			{
				system("cls");
				PrintBoard();
				cout << "请输入[" << i << " , " << j << "]这个格子的属主【0，1，2】输入99结束" << endl;
				int Num;
				cin >> Num;
				if (Num == 1)
					board[i][j] = 2;
...
//局面估值
bool Board::GetFreeBoxBool(int bx, int by)//判断某个格子是否为自由格
{
	//注意的是，输入的X与Y是Box实际地址
...
bool Board::GetLongCTypeBoxExist()//判断局面中是否存在长C型链
{
	int a = true;//++
...
void Board::GetAllCTypeBoxes(int Player,bool Msg)//吃掉所有的C型格
{
...
LOC Board::FindNextBox(LOC FoundBox, LOC LastBox)//查找某个链格
{
...
//后期计算
LOC Board::GetDoubleCrossLoc(int Player)//查找一个doublecross的制作方法，返回值为该边的坐标
{
	//得到可以制造双交的那个边的坐标
	for (int by = 1; by < LEN - 1; by = by + 2)
	{
...
bool Board::GetCTypeBoxLimit(int Player,bool Msg)//限制版吃C型格
{
	for (int y = 1; y < LEN - 1; y = y + 2)
	{
		for (int x = 1; x < LEN - 1; x = x + 2)//x轴
		{
			if (GetCTypeBoxBool(x, y))//检查是否为死格
			{
				int lx, ly;
...
int Board::GetFilterMoves(LOC Moves[MOVENUM])//得到所有的过滤可行边(不产生长链)
{
	int MoveNum = 0;
	for (int y = 0; y < LEN; y++)
	{
		for (int x = 0; x < LEN; x++)
		{
			if (board[x][y] == EDGE)//若为空白边
			{
				...
				BoardCopy(board, BoardSave);	//建造模拟棋盘
...
int Board::GetFilterMoveNum()//得到所有的过滤可行边的数量
{
	int MoveNum = 0;
	for (int y = 0; y < LEN; y++)
	{
		for (int x = 0; x < LEN; x++)
		{
			if (board[x][y] == EDGE)//若为空白边
			{
				...
				/*if (!GetLongCTypeBoxExist())	//判断局面中是否存在长C型链
												//若不存在长死格，也就是只有单链或者没有死格
				{
				//存入TURN
				Moves[MoveNum].Set(x, y);
				MoveNum++;//总数目自增
...
void MoveMsg(int x, int y, int Player)//搜索Msg坐标
{
	cout << "Msg: ";
	if (Player == RED)
		cprintf("Red", 12);
