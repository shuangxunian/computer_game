...
class playInfo():
    ...
    def update(self, r,c):  # 更新棋盘上的位置 更新下棋的策略 更新下棋的双方

        if self.board[r][c] != 0: # 判断此位置是否已有棋子
            return False
        self.board[r][c] = self.mode
        self.checkWinning(self.mode) # 更新下棋的策略
        self.mode = - self.mode # 交换下棋的双方
        return True
#？
    def checkWinning(self, player): # 检查是否双方有一方赢得比赛
