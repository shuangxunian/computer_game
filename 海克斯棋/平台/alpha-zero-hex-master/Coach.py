...
class Coach():
    ...
    def executeEpisode(self):
        ...
        while True:
            ...
            # print('======= E {} P {}'.format(episodeStep, self.curPlayer))
            # display(board)
            # print('conn board')
            ...
            # print('action', int(action/self.game.n), action%self.game.n)
    ...
    def learn(self):
        ...
        for i in range(startIter, self.args.numIters+1):
            ...
            arena = Arena(lambda x, player: np.argmax(pmcts.getActionProb(x, player, temp=0)),
                          lambda x, player: np.argmax(nmcts.getActionProb(x, player, temp=0)), self.game, display=display)
